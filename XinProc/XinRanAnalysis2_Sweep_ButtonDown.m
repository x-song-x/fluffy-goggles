...
if SpecUpdt
    ...
    SpecH =         getappdata(H,       'SpecH');
    SpecNum =       getappdata(SpecH,   'SpecNum');
    SpecAmp =       getappdata(SpecH,   'SpecAmp');
    SpecAgl =       getappdata(SpecH,   'SpecAgl');
    SpecDotH =      getappdata(SpecH,   'SpecDotH');
    SpecRepNumH =	getappdata(SpecH,   'SpecRepNumH');
...
%     HistH =         getappdata(SpecH,       'HistH');
    ...
    if SpecNum>size(SpecAmp,2)/2;   SpecNum = floor(size(SpecAmp,2)/2);	end    
    % update AxesSpec GUI & data
    setappdata(SpecH,   'SpecNum',  SpecNum);
    set(SpecDotH,       'XData',    double((SpecNum-1))/SpecSesDurTotal);
    set(SpecRepNumH,    'string',   sprintf('rep#:%d', SpecNum-1));
    % update AxesTune data
        RawHue =	SpecAgl(:,SpecNum)/(2*pi);
    if SpecNum == Spec_N_Qfc
        RawHue =	(RawHue-Spec_RatioCut_Qfc)/(1-2*Spec_RatioCut_Qfc);
    end
...
if TuneUpdt
    ...
    RawVal =        getappdata(TuningH,	'RawVal');

    HueMapOptions = getappdata(TuningH,	'HueMapOptions');
    HueMapOrder =   getappdata(TuningH,	'HueMapOrder');
    HueMap =        getappdata(TuningH,	'HueMap');
    HueTempolate =	getappdata(TuningH,	'HueTempolate');
    HueColorMap =   getappdata(TuningH,	'HueColorMap');
    HueAxesH =      getappdata(TuningH,	'HueAxesH');

    SatParaRange =  getappdata(TuningH, 'SatParaRange');
    SatParaOrder =  getappdata(TuningH,	'SatParaOrder');
    SatValSync =    getappdata(TuningH,	'SatValSync');
    SatGroundOut =  getappdata(TuningH,	'SatGroundOut');
    SatGroundTime =	getappdata(TuningH,	'SatGroundTime');
    SatStimTime =   getappdata(TuningH,	'SatStimTime');

    ValLimRange =   getappdata(TuningH,	'ValLimRange');
    ValLimOrder =   getappdata(TuningH,	'ValLimOrder');
    ValLim =        getappdata(TuningH,	'ValLim');
    ...
    switch ButtonTag(5:end)
        case 'ScalebarHue'  
            ...
            setappdata(TuningH,	'HueMapOrder',      HueMapOrder);
        ...
        case 'ScalebarSat'
            ...
            setappdata(TuningH,	'SatParaOrder',     SatParaOrder);
            setappdata(TuningH,	'SatValSync',       SatValSync);
            setappdata(TuningH,  'SatGroundOut',     SatGroundOut);
            setappdata(TuningH,	'SatGroundTime',	SatGroundTime);
    ...
    PtOne_Hue =             min(max(RawHue, 0), 1);             % HUE
    ...
    PtOne_Val =             min(RawVal/ValLim,1);               % VALUE
...
if TempAmpUpdt  
    ...
    TmaxRange =     getappdata(H,   'TmaxRange'); 
    AxesFrmeH =     getappdata(H,   'AxesFrmeH');
    AxesTempH =     getappdata(H,   'AxesTempH');
    AxesPixlH =     getappdata(H,   'AxesPixlH');
    TmaxIdx =       find(TmaxRange==Tmax);
    TmaxIdx =       TmaxIdx + 1;
    if TmaxIdx > length(TmaxRange)
        TmaxIdx =   1;
    end
    Tmax =          TmaxRange(TmaxIdx);
    TmaxLabels =    {	sprintf('-%3.1f%%',Tmax*100),...
                        '0%',...
                        sprintf(' %3.1f%%',Tmax*100) };
    caxis(AxesFrmeH,                Tmax*[-1 1]);
    set(H,          'Limits',       Tmax*[-1 1]);
    set(AxesTempH,	'YLim',         Tmax*[-1 1]);
    set(AxesPixlH,	'YLim',         Tmax*[-1 1]);
    set(H,          'Ticks',        Tmax*[-1 0 1]);
    set(AxesTempH,	'YTick',        Tmax*[-1 0 1]);
    set(AxesPixlH,	'YTick',        Tmax*[-1 0 1]);
    set(H,          'TickLabels',   TmaxLabels);
    set(AxesTempH,	'YTickLabels',  TmaxLabels);
...
if FrmeUpdt
    ...
    PlayingNow =    getappdata(XLabelH,     'PlayingNow');   
    % update FrmeNum
    ...
    set(XLabelH,            'String',   XLabelString);
...
if Play
    ...
    PlayingNow =    1 - PlayingNow;
    setappdata(H,               'PlayingNow',   PlayingNow);
    if PlayingNow
        FrmeNextH =     getappdata(H,           'FrmeNextH');
        Sound =         getappdata(H,           'Sound');
        FrmeImageH =    getappdata(FrmeNextH,	'FrmeImageH');
        TempHidH =      getappdata(FrmeNextH,	'TempHidH');
        FrmeDuration =	getappdata(TempHidH,	'FrmeDuration');
        TrialTotalFrmeNum =     get(TempHidH,   'XLim');
        TrialTotalFrmeNum =     TrialTotalFrmeNum(2);
        timelimit =             TrialTotalFrmeNum*FrmeDuration;
        hAP = audioplayer(Sound, 100e3);        
        FrmeNum =               0;  
        setappdata(H,           'AudioPlayerH', hAP); 
        setappdata(FrmeImageH,  'FrmeNum',      FrmeNum);    
        pause(0.5);
        timebase = tic;
        hAP.play;
        while toc(timebase)<timelimit && getappdata(H,	'PlayingNow')
            if toc(timebase)>(FrmeNum*FrmeDuration)
                XinRanAnalysis2_Sweep_ButtonDown(FrmeNextH);
                FrmeNum = FrmeNum + 1;
            end       
            pause(0.05);
        end
...
if PixlUpdt
    ...
    PwInd =             PixelPosi(1);
    AxesPixlH =         getappdata(H,           'AxesPixlH');
    AxesSpecH =         getappdata(H,           'AxesSpecH');
    ...
    TextPixlH =         getappdata(AxesPixlH,   'TextPixlH');
    LineSpecPixlH =     getappdata(AxesSpecH,   'LineSpecPixlH');
	SpecAmp =           getappdata(AxesSpecH,   'SpecAmp');
    PtInd =             PhInd + (PwInd-1)*size(FptCtPhPw_NormTrl,3);
    for i = 1:length(LineAllH)
        LineAllH(i).YData =    squeeze(FptCtPhPw_NormTrl(:,i,PhInd,PwInd));
    end
    LineMeanH.YData =	mean(squeeze(FptCtPhPw_NormTrl(:,:,PhInd,PwInd)),2);
    AxesPixlH.Title.String = sprintf('Pixel(%dH,%dW): temporal traces across reps',...
        PhInd,PwInd);
    TextPixlH.String =	{...
        sprintf('Across-all:      %5.2f%%', 100*std(reshape(FptCtPhPw_NormTrl(:,:,PhInd,PwInd), 1, []))),...
        sprintf('Across-rep:    %5.2f%%',	100*std(mean(FptCtPhPw_NormTrl(:,:,PhInd,PwInd), 2), 0, 1)),...
        sprintf('Across-frame:%5.2f%%',     100*std(mean(FptCtPhPw_NormTrl(:,:,PhInd,PwInd), 1), 0, 2)),...
        'on STD'};
    LineSpecPixlH.YData =   SpecAmp(PtInd, :);
    drawnow;
end
%% Title clipping
if TitlUpdt
    ...
    switch ButtonTag(5:8)
        case 'Var1';    AxesPlus = [    -2  -56 54  17  ];
        case 'Var2';    AxesPlus = [    -2  -56 64  17  ];
        case 'Var3';    AxesPlus = [    -2  -56 64  17  ];
        case 'Temp';    AxesPlus = [    -58 -35 10  17  ];
        case 'Pixl';    AxesPlus = [    -58 -35 10  17  ];
        case 'Spec';    AxesPlus = [    -70 -35 10  17  ];
        case 'Tune';    AxesPlus = [    -5  -25 100 17  ];
        case 'TuIm';    AxesPlus = [    0   0   0   0   ];
        case 'Frme';    AxesPlus = [    -67 -25 0   17  ];
        case 'Hist';    AxesPlus = [    -55 -57 5   17  ];
        otherwise
    end
    AxesAdd =       AxesPlus;
